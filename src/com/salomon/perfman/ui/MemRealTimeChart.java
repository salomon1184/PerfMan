...
public class MemRealTimeChart extends ChartPanel implements Runnable {
	...
	public void run() {
		while (true) {
			try {
				...
				long memData = this.utility.getMemInfoValue(this.phone,
						this.packName);
				timeSeries.add(new Millisecond(), memData);
				this.recorder.writePerfDataToFile(Long.toString(memData));
